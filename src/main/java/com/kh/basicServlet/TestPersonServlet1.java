...
public class TestPersonServlet1 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// 사용자의 요청 방식이 Get인 경우 실행되는 메소드
		/*
		 * 1. HttpServletRequest - 클라이언트가 요청 시 전달한 데이터가 담겨 있는 객체 - 사용자가 입력한 값, 요청 시 전송
		 * 방식, ip 주소, 기타...
		 * 
		 * 2. HttpServletResponse - 응답 정보를 담아줄 객체 - 응답 정보로 동적 html 페이지를 생성하여 반환
		 */
		// 1. 사용자가 입력한 값 가져오기
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		// 사용자의 요청 방식이 Post인 경우 실행되는 메소드
		/*
		 * POST 방식의 요청 시 데이터는 HTTP BODY 영역에 "인코딩"되어 넘어옴
		 * POST 방식의 기본 인코딩 설정은 ISO-8859-1로, utf-8이 아님
		 * 따라서 Request의 인코딩 타입을 utf-8로 변경해야 함
		 */
