...
public class ElServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 		- ServletContext는 서버가 멈추기 전까지 유지됨
		 * 
		 * 2. HttpSession(session scope)
		 * 		- 접속한 사용자별로 데이터를 저장할 수 있는 객체
		 * 		- 개발자가 HttpSession 지속 시간을 설정할 수 있음
		 * 		- 이 영역에 데이터를 추가하면 모든 servlet에서 사용이 가능
		 * 		- 값을 한번 담으면 session의 지속시간이 끝나거나, 브라우저가 닫히거나,
		 * 		  서버가 종료될 때까지 데이터가 유지됨
		 * 
		 * 3. HttpServletRequest(request scope)
		 * 		- 클라이언트가 요청을 보낼 때마다 새롭게 생성되는 객체
		 * 		- 이 영역에 데이터를 담으면 현재 요청을 받은 서블릿과 포워딩 받는 서블릿/jsp에서
		 * 		  request에 담긴 데이터를 사용할 수 있음
		 * 		- 값이 한번 담기면, 재요청이 이뤄지는 순간 이전 request가 소멸
		 * 
		 * 4. PageContext(page scope)
		 * 		- JSP 페이지에서만 사용 가능한 영역, 하나의 JSP 페이지에서만 공유할 데이터를 선언하는 영역
		 * */
